...
if uploaded_file is not None:
    ...
    if df.empty:
        ...
        if st.sidebar.button("Show Analysis"):
            num_messages,num_words,num_media,num_link,num_del,num_edi= helper.fetch_stats(selected_user, df)
        ...
        # deleted messages
            ...
            with col1:
                st.header("number of Messages Deleted")
                st.title(num_del)
            with col2:
                st.header("number of Messages Edited")
                st.title(num_edi)
            ...
            # now we will go with the total timeline trend or line graph
            time = helper.timeline(selected_user, df)
            st.title("Timeline")
            fid, ax = plt.subplots()
            ax.plot(time['time'], time['message'])
            plt.xticks(rotation='vertical')
            st.pyplot(fid)


            # how about some daily time line:
            dtf = helper.dating(selected_user, df)
            st.title("daily timeline")
            fig,ax = plt.subplots()
            ax.plot(dtf['only_date'], dtf['message'])
            plt.xticks(rotation='vertical')
            st.pyplot(fig)
